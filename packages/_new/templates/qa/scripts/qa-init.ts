...
const ensurePackageJson = async (
  ...
  qaPackageName: string,
) => {
  const scripts = (pkg.scripts ?? {}) as Record<string, string>
  const devDependencies = (pkg.devDependencies ?? {}) as Record<string, string>

  applyQaScripts(scripts, kind)
  applyQaDevDependencies(devDependencies, kind, qaPackageName)
  await writePackageJson({ devDependencies, dir, pkg, scripts })
}

const ensurePrettierConfig = async (dir: string, tailwind: boolean, force: boolean, qaPackageName: string) => {
  ...
  if (existing.includes('/qa/prettier')) {
    await writeFile(configPath, contents, 'utf8')
  }
}

const ensureOxlintConfig = async (dir: string, force: boolean, qaPackageName: string) => {
  const configPath = path.join(dir, 'oxlint.json')
  if (!existsSync(configPath) || force) {
    const contents = `{
  "$schema": "../../node_modules/oxlint/configuration_schema.json",
  "extends": ["${qaPackageName}/oxlint"]
}\n`
    await writeFile(configPath, contents, 'utf8')
  } else {
    await updateOxlintConfig(configPath, qaPackageName)
  }
}

const resolveTsconfigPreset = (kind: Exclude<Kind, 'auto'>, pkg: Record<string, unknown>, qaPackageName: string) => {
  if (kind === 'web') {
    return `${qaPackageName}/tsconfig/web`
  }
  // CLI packages may use React (e.g. Ink) but still want "node" defaults (no DOM libs).
  if (kind === 'cli') {
    return `${qaPackageName}/tsconfig/node`
  }
  const isReact = detectReact(pkg)
  if (isReact) {
    return `${qaPackageName}/tsconfig/react-lib`
  }
  return `${qaPackageName}/tsconfig/node`
}
...
const ensureTsconfig = async ({ dir, kind, pkg, force, qaPackageName }: TsconfigOptions) => {
  const preset = resolveTsconfigPreset(kind, pkg, qaPackageName)
  ...
  const currentExtends = existing.extends
  const shouldReplace =
    force ||
    !currentExtends ||
    (typeof currentExtends === 'string' && /\/qa\/tsconfig\//.test(currentExtends) && currentExtends !== preset)
  if (shouldReplace) {
...
const main = async () => {
  ...
  await ensurePackageJson(dir, pkg, resolvedKind, qaPackageName)
  await ensurePrettierConfig(dir, resolvedTailwind, options.force, qaPackageName)
  await ensureOxlintConfig(dir, options.force, qaPackageName)
  await ensureTsconfig({ dir, force: options.force, kind: resolvedKind, pkg, qaPackageName })
...
const applyQaDevDependencies = (
  devDependencies: Record<string, string>,
  kind: Exclude<Kind, 'auto'>,
  qaPackageName: string,
) => {
  for (const dep of Object.keys(devDependencies)) {
    if (dep !== qaPackageName && dep.endsWith('/qa')) {
      delete devDependencies[dep]
    }
  }
  devDependencies[qaPackageName] = 'workspace:*'
  if (kind !== 'web') {
...
const resolvePrettierTarget = (tailwind: boolean, qaPackageName: string) => {
  let target = `${qaPackageName}/prettier`
  if (tailwind) {
    target = `${qaPackageName}/prettier-tailwind`
  }
  return target
}

const updateOxlintConfig = async (configPath: string, qaPackageName: string) => {
  const config = await readJson<Record<string, unknown>>(configPath)
  const extendsField = coerceExtendsField(config.extends)
  const oxlintExtends = `${qaPackageName}/oxlint`

  const normalized = extendsField
    .map((value) => (value.endsWith('/qa/oxlint') ? oxlintExtends : value))
    .filter((value, index, all) => all.indexOf(value) === index)
  if (!normalized.includes(oxlintExtends)) {
    normalized.push(oxlintExtends)
  }

  config.$schema = '../../node_modules/oxlint/configuration_schema.json'
  config.extends = normalized
