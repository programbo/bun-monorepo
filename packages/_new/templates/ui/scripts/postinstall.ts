...
const QA_DIR = path.join(ROOT_DIR, 'packages', 'qa')
const QA_TEMPLATE_DIR = path.join(ROOT_DIR, '.bun-create', 'qa')
...
const ensureQaPackage = async () => {
  ...
  await run('bun', ['create', 'qa', 'packages/qa', '--no-install', '--no-git'], ROOT_DIR)
}

const runQaInit = async () => {
  await ensureQaPackage()
  if (!existsSync(QA_DIR)) return
  await run('bun', ['run', '--cwd', QA_DIR, 'qa:init', '--dir', PACKAGE_DIR, '--kind', 'lib', '--tailwind'], ROOT_DIR)
}
