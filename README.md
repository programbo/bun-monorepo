...
This repo exists to give you a fast, batteries‑included Bun workspace with a production‑ready React + Tailwind web app, plus opinionated scaffolding and QA tooling so every package starts clean.
...
- **Seamless dev server takeover**: If a matching app is already running on the default port, the `bun new` process gracefully stops it and takes over without manual cleanup.
- **`packages/_new`**: Scaffolds apps and packages with the repo’s QA defaults baked in.  
  Run: `bun new <web|api|cli|lib|ui>`
...
- The `bun new` scaffolder injects QA defaults into generated workspaces.
